...
impl Cdrom {
    ...
    fn handle_command(&mut self, cmd: u8) {
        ...
        match cmd {
            ...
            0x10 => {
                ...
                log::info!("cdrom cmd: GetLocL");
                let response = self.last_locl_response;
    ...
    fn do_seek(&mut self) {
        if let Some(params) = self.set_loc_params {
            ...
            self.last_locl_response[0] = to_bcd(minutes as u8);
            self.last_locl_response[1] = to_bcd(seconds as u8);
            self.last_locl_response[2] = to_bcd(sector as u8);
            for byte in &mut self.last_locl_response[3..] {
                *byte = 0;
            }
